...
export default class ProductService {
  async add(data: []): Promise<ServiceSuccessResponse | ServiceFailedResponse> {
    try {
      ...
      AvailabilityModel.insertMany(data)
      ...
      //   const savedData = await AvailabilityModel.updateOne({ aw_product_id: data.aw_product_id }, data)
      //   return ServiceResponse.success({
      //     data: savedData,
      //     status: 201,
      //   })
  ...
  async getByKey(key: any) {
    const data: any = await AvailabilityModel.findOne(key)
    if (data) {
      return data._doc
    }
