...
export default class TopSellersService {
  async getAllCurrentBrandImages() {
    const record = await TopSeller.find()
    console.log(record)
    if (record) {
  ...
  async getTopSellerData(brand_type:any) {
    const record = await TopSeller.aggregate([
      {
        $match:{
          $or: [
  ...
  async addProduct(
    ...
    try {
      const findOne = await TopSeller.findOne({top_seller:data.brand_name,brand_type:data.brand_type});
      if(findOne){
        return ServiceResponse.failed({
          code: ErrorCodes.USER_NOT_FOUND,
          data: {
      ...
      const savedData = await TopSeller.create({
  ...
  async deleteBrand(id: string): Promise<ServiceSuccessResponse | ServiceFailedResponse> {
    const result = await TopSeller.findByIdAndRemove({
      _id: id,
    })
    if (result) {
      return ServiceResponse.success({
