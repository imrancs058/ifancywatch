...
import TopWatchStrapSellersService from "../../services/Top-watch-strap-sellers"
...
@Controller("/top-watch-strap-sellers")
export default class TopWatchStrapController {
  @Route.Get({ path: "/" ,middlewares:[util.ensureAuthenticated] })
  async ProductCategoryPage(req: Request, res: Response) {
    const brandNames = await new BrandImagesService().getAllWatchStrapBrandNames()
    // const categoryNames = await new CategorySelectService().getAllProductCategories()
    return res.render("top-watch-strap-sellers/index", { data: brandNames })
  }

  @Route.Get({ path: "/get_top_seller" })
  async getImageData(req: Request, res: Response) {
    console.log("===================================")
    const record = await new TopWatchStrapSellersService().getAllCurrentBrandImages()
  ...
  async addPage(req: Request, res: Response) {
    ...
    const result = await new TopWatchStrapSellersService().addProduct(req.body)
    if (result.status == 0) {
      return res.send('<script>alert("Brand already exist");window.location.href="/top-sellers"</script>')
    }else {
      return res.redirect("/top-watch-strap-sellers")
    }
  }
  ...
  async deleteRecord(req: Request, res: Response) {
    await new TopWatchStrapSellersService().deleteBrand(req.params._id)
    return res.redirect("/top-watch-strap-sellers")
  }
}
