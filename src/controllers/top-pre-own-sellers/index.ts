...
import TopPreOwnedSellersService from "../../services/Top-Pre-Own-Sellers"
...
@Controller("/top-pre-own-sellers")
export default class TopPreOwnController {
  ...
  async ProductCategoryPage(req: Request, res: Response) {
    ...
    return res.render("top-pre-own-sellers/index", { data: brandNames })
  }

  @Route.Get({ path: "/get_top_seller" })
  async getImageData(req: Request, res: Response) {
    const record = await new TopPreOwnedSellersService().getAllCurrentBrandImages()
  ...
  async addPage(req: Request, res: Response) {
    ...
    const result = await new TopPreOwnedSellersService().addProduct(req.body)
    if (result.status == 0) {
      return res.send('<script>alert("Brand already exist");window.location.href="/top-sellers"</script>')
    }else {
      return res.redirect("/top-pre-own-sellers")
    }
  }
  ...
  async GetTopSellerData(req: Request, res: Response) {
    ...
    const record = await new TopPreOwnedSellersService().getTopSellerData(brand_type)
    return res.json(record)
  }

  @Route.Get({ path: "/deletebrand/:_id" })
  async deleteRecord(req: Request, res: Response) {
    await new TopPreOwnedSellersService().deleteBrand(req.params._id)
    return res.redirect("/top-pre-own-sellers")
  }
}
